...
import type { ConversationWithLastMessage, Message, Patient } from "@/lib/types";
...
export default function Dashboard() {
  ...
  const fetchPatient = useCallback(async (convoId: string) => {
    const res = await fetch(`/api/patients/${convoId}`);
    if (res.ok) {
      const data = await res.json();
      setPatient(data);
    } else {
  ...
  }, [selectedId, fetchMessages, fetchPatient]);
  ...
  async function updatePatient(updates: Partial<Patient>) {
  ...
  return (
    <div className="flex h-screen bg-[#0f0f0f] font-sans">
      ...
      {/* CRM Panel */}
      {selected && (
        <div className="w-[380px] flex flex-col border-l border-white/[0.06]" style={{ background: "#141414" }}>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {!patient ? (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                <div className="text-center">
                  <p className="text-sm text-white/40">No patient profile found</p>
                  <p className="text-xs text-white/20 mt-1">Create a profile to start tracking CRM data</p>
                </div>
                <button
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] uppercase tracking-wider text-white/30 font-bold">Basic Information</span>
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                      patient.status === 'patient' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-500/20 text-blue-400'
                    }`}>
                      {patient.status}
                    </span>
                  </div>
                  <div className="grid grid-cols-1 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500/40 outline-none transition-colors"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-white/40 block">Email</label>
                      <input
                        value={patient.email || ""}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[10px] text-white/40 block">DOB</label>
                        <input
                          type="date"
                          value={patient.date_of_birth || ""}
                          onChange={(e) => updatePatient({ date_of_birth: e.target.value })}
                          className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500/40 outline-none transition-colors"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[10px] text-white/40 block">Status</label>
                        <select
                          value={patient.status}
                          onChange={(e) => updatePatient({ status: e.target.value as any })}
                          className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500/40 outline-none transition-colors"
                        >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] uppercase tracking-wider text-white/30 font-bold">Clinical Details</span>
                  </div>
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-swhite/40 block">Insurance Provider</label>
                      <input
                        value={patient.insurance_provider || ""}
                        onChange={(e) => updatePatient({ insurance_provider: e.target.value })}
                        className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500/40 outline-none transition-colors"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-white/40 block">Medical History</label>
                      <textarea
                        value={patient.medical_history || ""}
                        onChange={(e) => updatePatient({ medical_history: e.target.value })}
                        rows={4}
                        className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500/40 outline-none transition-colors resize-none"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-white/40 block">CRM Notes</label>
                      <textarea
                        value={patient.notes || ""}
